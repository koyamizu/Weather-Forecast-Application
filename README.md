...
ただ、やはりChatGPTによる処理が介在する分、初回の検索は少々時間がかかります。ChatGPT5の各モデルを比較したところ、miniが一番速かったのですが、それでも10秒~20秒かかる時があります。少しでも処理を早くするため、GPTが返した「ユーザーの入力値」「都市名-行政区画名」「緯度経度」をセットでデータベースに保存することにしました。これによって、重複地名であっても、再びGPTの処理を介することなく、都市名や緯度経度を利用できるようになりました。
