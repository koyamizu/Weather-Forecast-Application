...
@MybatisTest
@Transactional
public class WeatherForecastMapperTest {
	...
	void test_insertDay() {
		...
		mapper.insertDay(LocalDate.now(), location.getCityRegion(), dummy);
	}
	...
	void test_insertTime() {
		...
		mapper.insertHour(LocalDate.now(), location.getCityRegion(), dummies);
	}
	...
	void test_selectDay() {
		...
		ForecastDay actual = mapper.selectDay("新宿区-東京都", LocalDate.parse("2025-09-07"));
	...
	void test_selectHour() {
		...
		List<ForecastHour> actual = mapper.selectHour("新宿区-東京都", LocalDate.parse("2025-09-07"));
		assertThat(actual).isEqualTo(expected);
	}
